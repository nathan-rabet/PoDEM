...
class Verbe:
    ...
    def groupe(self):
        ...
        if self.txt != "aller" and self.txt[-2::] == "er":
            return 1
        #Deuxième groupe
        elif self.txt in open(environ["POETRY_PATH"] + "/M/vb2g.txt").read():
            return 2
        #Troisième groupe
        else:
            return 3
    ...
    def conjuguer(self, pers="1p", nombre="s", genre="m", temps="présent", pronom=True):
        ...
        temps => temps souhaité, écrit comme dans la base de données
        pers => 1p ou 2p
        ...
        if self.groupe() == 3:
            #A compléter après la mise en place de la base de données
            return None
